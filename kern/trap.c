...
page_fault_handler(struct Trapframe *tf)
{
	...
	if(tf->tf_esp <= UXSTACKTOP - 1 && tf->tf_esp >= UXSTACKTOP - PGSIZE)
		stpoint = (tf->tf_esp - sizeof(struct UTrapframe) - 4);
	else 
		stpoint = UXSTACKTOP - sizeof(struct UTrapframe);
	utrap = (struct UTrapframe* ) stpoint;
	...
	user_mem_assert(curenv, (void *)stpoint, sizeof(struct UTrapframe), PTE_P | PTE_W | PTE_U);
